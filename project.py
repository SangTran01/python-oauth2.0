...
def showRestaurants():
  ...
  Robo = session.query(User).filter_by(email='tinnyTim@udacity.com').one()
  print '-------------------'
  print "Robo id is %s" % Robo.id
...
def newMenuItem(restaurant_id):
  ...
  if restaurant.user_id != login_session['user_id']:
    return "<script>alert('NAH BRO')</script>"
  if request.method == 'POST':
...
# def getUserByRestaurant(RestaurantUserId):
#   try:
#     user = session.query(Restaurant).filter_by(user_id = RestaurantUserId).one()
#     return user.id
#   except:
#     return None
